...
namespace Ibuprogames
{
  namespace OilPaintAsset
  {
    ...
    /// About window.
    /// </summary>
    public sealed class OilPaintAbout : EditorWindow
    {
      public static readonly string assetID = @"Ibuprogames.OilPaint";
      private static readonly string description = @"<b>Oil Paint</b> is a post-processing effect that can turn your game into a piece of art. It is based on Anisotropic Kuwahara filter.";

      private static readonly string documentationURL = @"http://www.ibuprogames.com/2015/05/04/oil-paint-image-effect/";
      private static readonly string publisherURL = @"https://assetstore.unity.com/publishers/8484";
      private static readonly string forumURL = @"https://forum.unity.com/threads/released-oil-paint-image-effect.277980/";
      private static readonly string storeURL = @"https://assetstore.unity.com/packages/vfx/shaders/fullscreen-camera-effects/oil-paint-24174";
      ...
      [MenuItem("Help/Ibuprogames/Oil Paint/About", false, 0)]
      public static void MenuAbout()
      {
        Open();
      }

      [MenuItem("Help/Ibuprogames/Oil Paint/Documentation", false, 0)]
      ...
      {
        string[] results = AssetDatabase.FindAssets(@"OilPaintAbout t:Script", null);
        if (results.Length > 0)
        {
          string assetPath = AssetDatabase.GUIDToAssetPath(results[0]);
          assetPath = assetPath.Replace(@"Scripts/Editor/About/OilPaintAbout.cs", string.Empty);
      ...
      {
        OilPaintAbout window = EditorWindow.GetWindow<OilPaintAbout>(true, @"About 'Oil Paint'", true);
        ...
        GUILayout.BeginVertical(@"box");
        {
          ...
          GUILayout.BeginHorizontal(GUILayout.ExpandWidth(true));
          {
            GUILayout.BeginVertical();
            {
              ...
              if (GUILayout.Button(@" <b>Rate</b> 'Oil Paint', thanks!", buttonStyle, GUILayout.ExpandHeight(true)) == true)
    ...
    /// </summary>
    public sealed class OilPaintAboutProcessor : AssetPostprocessor
    {
      static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
      {
        ...
        {
          if (importedAssets[i].Contains(@"OilPaint/Changelog.txt") == true && importedAssets[i].EndsWith(@".meta") == false)
          {
            string assetPath = OilPaintAbout.GetAssetPath();
            if (string.IsNullOrEmpty(assetPath) == false)
            {
              TextAsset changeLogAsset = AssetDatabase.LoadAssetAtPath<TextAsset>(assetPath + @"Changelog.txt");
              if (changeLogAsset != null && string.IsNullOrEmpty(changeLogAsset.text) == false)
              {
                ...
                if (EditorPrefs.GetString(OilPaintAbout.assetID, @"0.0") != version)
                  OilPaintAbout.Open();
